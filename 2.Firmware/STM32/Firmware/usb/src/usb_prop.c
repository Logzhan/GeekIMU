...
void CustomHID_init(void)
{
  ...
  ID*/
  Get_SerialNum((char *)CustomHID_StringSerial, 0);
...
//#else
  ...
  SetEPTxCount(ENDP1, 62);
  SetEPRxCount(ENDP1, 62);
    ...
    switch (RequestNo) {
      ...
      case GET_REPORT:
        ...
        if (pInformation->USBwValue1 == HID_FEATURE)
          CopyRoutine = feature_config_getreport;
        break;

      case SET_REPORT:
        if (pInformation->USBwValue1 == HID_FEATURE) {
          Request = SET_REPORT;
          CopyRoutine = feature_config_setreport;
        }
        break;
